...
function makeRange(contentReference: ContentReference, startPoint: Point, endPoint: Point, id: string): Range {
    if (
        (isBlockPoint(startPoint) &&
            ...
            areBlockReferencesAtSameBlock(startPoint.blockReference, endPoint.paragraphBlockReference)) ||
        (isParagraphPoint(startPoint) &&
            isBlockPoint(endPoint) &&
            areBlockReferencesAtSameBlock(startPoint.paragraphBlockReference, endPoint.blockReference)) ||
...
> {
    ...
    function applyViewDelta(viewDelta: ViewDelta): void {
        groupConsecutiveItemsInArray(
            ...
            (a, b) => {
                if (
                    a.type === ViewDeltaChangeType.BlockConfigOrParagraphChildrenUpdated &&
                    b.type === ViewDeltaChangeType.BlockConfigOrParagraphChildrenUpdated
                ) {
...
function makeRangesConnectingPointsAtContentReferences(
    ...
    if (
        (isBlockPoint(startPoint) &&
            !areContentReferencesAtSameContent(
                ...
                makeContentReferenceFromContent(accessContentFromBlockPoint(document, startPoint)),
            )) ||
        (isParagraphPoint(startPoint) &&
            !areContentReferencesAtSameContent(
                startPointContentReference,
                makeContentReferenceFromContent(accessContentFromParagraphPoint(document, startPoint)),
            ))
    ) {
    ...
    for (let i = sortedStartPointParentContentReferencesWithEmbedReferences.length - 1; i > lastSharedParentContentReferenceIndex; i--) {
        ...
        if (
            !areBlockReferencesAtSameBlock(embedReference, makeBlockReferenceFromBlock(accessLastBlockInContentAtContentReference(document, contentReference)))
        ) {
            ranges.push(
                makeRange(
                    contentReference,
    ...
    if (lastSharedParentContentReferenceIndex === -1) {
                    ...
                    isStartOfContentPoint(topLevelStartPoint) ||
                        ...
                        (isParagraphPoint(topLevelStartPoint) &&
                            !areBlockReferencesAtSameBlock(
                                makeBlockReferenceFromParagraphPoint(topLevelStartPoint),
                                blockReferenceAtPreviousBlockToTopLevelEndBlock,
                            ))
                ...
                makeRange(
                    ...
                    isEndOfContentPoint(topLevelEndPoint) ||
                        ...
                        (isParagraphPoint(topLevelEndPoint) &&
                            !areBlockReferencesAtSameBlock(
                                makeBlockReferenceFromParagraphPoint(topLevelEndPoint),
                                blockReferenceAtNextBlockToTopLevelStartBlock,
                            ))
        ...
        const sharedParentSortedEndEmbedReference =
            sortedEndPointParentContentReferencesWithEmbedReferences[lastSharedParentContentReferenceIndex].embedReference;
        if (
            !areBlockReferencesAtSameBlock(
        ...
        const indexToTimeTravelFrom =
            currentTimeTravelInfo_ === null
                ...
                : committedMutationInfosForReversing.findIndex(
                      (committedMutationInfoForReversing) =>
                          committedMutationInfoForReversing.mutationId === currentTimeTravelInfo_.timeTraveledToAfterMutationId,
                  );
    ...
    switch (mutation.type) {
                    ...
                    mutation.type === MutationType.InsertBlocksBefore
                        ? blockIndex === 0
                            ...
                            : makeBlockReferenceFromBlock(
                                  accessPreviousBlockToBlockAtBlockReference(document, makeBlockReferenceFromBlockPoint(insertionBlockPoint)),
                              )
        ...
        case MutationType.MoveContentsAtEnd: {
            ...
            const transformSelectionRange =
                ...
                makeRemoveContentsSelectionTransformFn(document, startContentReference, () => contentListFragment, stateViewAfterMutation);
                    ...
                    contentReference:
                        mutation.type === MutationType.MoveContentsAfter
                            ...
                            ? areContentReferencesAtSameContent(
                                ...
                                ? null
                                : makeContentReferenceFromContent(
                                      accessPreviousContentToContentInEmbedAtContentReference(document, mutation.moveBeforeContentReference),
                                  )
                            : getNumberOfEmbedContentsInEmbedAtBlockReference(document, embedReference) === 0
                            ? null
                            : makeContentReferenceFromContent(accessLastContentInEmbedAtBlockReference(document, embedReference)),
            ...
            if (mutation.type !== MutationType.MoveContentsAtEnd) {
                ...
                moveIntoEmbed.contentReferences.push(
                    ...contentListFragment.contentListFragmentContents.map(({ content }) => makeContentReferenceFromContent(content)),
                );
            }
            ...
            const nextContentReference = makeContentReferenceFromContent(
                accessContentAtIndexInEmbedAtBlockReference(document, embedReference, endContentIndex + 1),
            );
            return makeChangedMutationResult(
        ...
        case MutationType.RemoveBlocks: {
            ...
            if (startBlockIndex > 0) {
                ...
                return makeChangedMutationResult(
                    makeBatchMutation([makeInsertBlocksAfterMutation(previousBlockPoint, contentFragment)]),
                    transformSelectionRange,
                );
            }
            if (endBlockIndex === 0) {
                return makeChangedMutationResult(
                    makeBatchMutation([makeInsertBlocksAtEndMutation(contentReference, contentFragment)]),
                    transformSelectionRange,
                );
            }
            const nextBlockPoint = makeBlockPointFromBlockReference(
        ...
        case MutationType.SpliceParagraph: {
            ...
            const transformSelectionRange: TransformSelectionRangeFn = (selectionRange) => {
                return makeSelectionRange(
                    selectionRange.ranges.map((range) => {
                        ...
                        return makeRange(
                            contentReference,
                            isParagraphPoint(startPoint) && isParagraphPointAtParagraph(startPoint, paragraph)
                                ? transformParagraphPointAtParagraph(startPoint)
                                : startPoint,
                            isParagraphPoint(endPoint) && isParagraphPointAtParagraph(endPoint, paragraph)
                                ? transformParagraphPointAtParagraph(endPoint)
                                : endPoint,
                            range.id,
                        );
                    }),
            ...
            return makeChangedMutationResult(
                makeBatchMutation([
                    makeSpliceParagraphMutation(paragraphPoint, getLengthOfParagraphInlineNodes(insertChildren), removedInlineNodes.map(cloneInline)),
                ]),
                transformSelectionRange,
            );
        }
        ...
        case MutationType.ChangeTextConfigBetweenParagraphPoints: {
            ...
            const formatTextBetween = (
                ...
                function formatPortionOfTextInParagraph(
                    paragraph: Paragraph<ParagraphConfig, TextConfig, VoidConfig>,
                    fromOffset: number,
                    toOffset: number,
                ): void {
                        ...
                        for (let j = 0; j < block.children.length; j++) {
                            ...
                            if (isText(inline) && !areNodeConfigsEqual(inline.config, newTextConfig)) {
                                ...
                                textsBetweenParagraphPoints.push({
                                    ...
                                    config: inline.config,
                                    startParagraphPoint: makeParagraphPointFromParagraphBlockReferenceAndOffset(paragraphReference, startInlineOffset),
                                    endParagraphPoint: makeParagraphPointFromParagraphBlockReferenceAndOffset(
                                        paragraphReference,
                                        startInlineOffset + inlineLength,
                                    ),
                                });
                ...
                const reverseMutations: Mutation<DocumentConfig, ContentConfig, ParagraphConfig, EmbedConfig, TextConfig, VoidConfig>[] =
                    groupConsecutiveItemsInArray(
        ...
        case MutationType.ChangeParagraphConfigBetweenBlockPoints: {
            ...
            const reverseMutations: Mutation<DocumentConfig, ContentConfig, ParagraphConfig, EmbedConfig, TextConfig, VoidConfig>[] =
                groupConsecutiveItemsInArray<number, ChangedGroupInfo | NotChangedGroupInfo>(
        ...
        case MutationType.ChangeContentConfig: {
            ...
            if (!areNodeConfigsEqual(previousContentConfig, newContentConfig)) {
                ...
                return makeChangedMutationResult(
                    makeBatchMutation([makeChangeContentConfigMutation(contentReference, cloneNodeConfig(previousContentConfig))]),
                    null,
                );
            }
            return makeNoChangeMutationResult();
        ...
        case MutationType.ChangeEmbedConfig: {
            ...
            if (!areNodeConfigsEqual(previousEmbedConfig, newEmbedConfig)) {
                ...
                return makeChangedMutationResult(
                    makeBatchMutation([makeChangeEmbedConfigMutation(embedBlockPoint, cloneNodeConfig(previousEmbedConfig))]),
                    null,
                );
            }
            return makeNoChangeMutationResult();
        ...
        case MutationType.ChangeVoidConfig: {
            ...
            if (!areNodeConfigsEqual(previousVoidConfig, newVoidConfig)) {
                ...
                return makeChangedMutationResult(
                    makeBatchMutation([makeChangeVoidConfigMutation(voidStartParagraphPoint, cloneNodeConfig(previousVoidConfig))]),
                    null,
                );
            }
            return makeNoChangeMutationResult();
                    ...
                    if (secondParagraphIndex - firstParagraphIndex > 1) {
                        mutations.push(
                            makeRemoveBlocksMutation(
                                makeBlockPointFromBlockReference(
                                    makeBlockReferenceFromBlock(
                                        accessBlockAtIndexInContentAtContentReference(document, contentReference, firstParagraphIndex + 1),
                                    ),
                                ),
                                makeBlockPointFromBlockReference(
                                    makeBlockReferenceFromBlock(
                                        accessBlockAtIndexInContentAtContentReference(document, contentReference, secondParagraphIndex - 1),
                                    ),
                                ),
                            ),
                        );
...
>(
    ...
    return (stateControl): void => {
        ...
        const accessors: MatchRangeOnPointTypesWithoutDirectionAccessors<void> = {
            ...
            EndOfContent_Block(_endOfContentPoint, blockPoint) {
                mutations.push(
                    makeRemoveBlocksMutation(blockPoint, makeBlockPointFromBlock(accessLastBlockInContentAtContentReference(document, contentReference))),
                );
                if (
                    areBlockPointsAtSameBlock(blockPoint, makeBlockPointFromBlock(accessBlockAtIndexInContentAtContentReference(document, contentReference, 0)))
                ) {
                    mutations.push(makeInsertBlocksAtEndMutation(contentReference, contentFragment));
                } else {
                    mutations.push(
                ...
                if (isParagraph(firstContentFragmentBlock)) {
                    ...
                    if (secondParagraphPoint.offset !== firstParagraphPoint.offset || firstContentFragmentBlockLength > 0) {
                        mutations.push(
                            makeSpliceParagraphMutation(
                                firstParagraphPoint,
                                secondParagraphPoint.offset - firstParagraphPoint.offset,
                                firstContentFragmentBlock.children,
                            ),
                        );
                    }
                    if (
                        ...
                        mutations.push(
                            makeChangeParagraphConfigBetweenBlockPointsMutation(paragraphBlockPoint, paragraphBlockPoint, firstContentFragmentBlock.config),
                        );
                    }
                    ...
                    if (isParagraph(lastContentFragmentBlock)) {
                        ...
                        mutations.push(
                            makeSplitParagraphForwardsMutation(
                                ...
                                getShouldReplaceEdgeParagraphConfig?.(
                                    dummyParagraph,
                                    lastContentFragmentBlock,
                                    0,
                                    secondParagraphPoint.offset,
                                    EdgeParagraphSide.End,
                                )
                        ...
                        if (contentFragmentBlocks.length > 2) {
                            mutations.push(
                                makeInsertBlocksAfterMutation(
                                    paragraphBlockPoint,
                                    makeContentFragment(contentFragmentBlocks.slice(1, contentFragmentBlocks.length - 1)),
                                ),
                            );
                        }
                        if (shouldTransformStateSelectionRangeToEndPredicateFn) {
                    ...
                    if (isParagraph(lastContentFragmentBlock)) {
                        ...
                        mutations.push(
                            makeInsertBlocksAfterMutation(
                                paragraphBlockPoint,
                                makeContentFragment(contentFragmentBlocks.slice(0, contentFragmentBlocks.length - 1)),
                            ),
                        );
                        if (shouldTransformStateSelectionRangeToEndPredicateFn) {
                            const endPoint = makeParagraphPointFromParagraphBlockReferenceAndOffset(
        ...
        return matchPointOnType<PointWithContentReference>(point, {
            ...
            Paragraph(point) {
                    ...
                    if (point.offset === 0) {
                        const newPoint =
                            accessLastPreviousPointToBlockAtBlockReference(
                                document,
                                makeBlockReferenceFromParagraphPoint(point),
                                SelectionRangeIntention.Text,
                            ) ?? point;
                ...
                if (pointMovement === PointMovement.Previous || pointMovement === PointMovement.PreviousBoundByEdge) {
                    ...
                    while (
                        (inlineWithStartOffset = getInlineNodeWithStartOffsetAfterParagraphPoint(
                            document,
                            changeParagraphPointOffset(point, inlineEndOffset),
                        )) &&
                    ...
                    if (!segmentPrevious) {
                        if (textStartOffsetInParagraph === 0) {
                            ...
                            return {
                                contentReference: makeContentReferenceFromContent(
                                    isBlockPoint(newPoint)
                                        ? accessContentFromBlockPoint(document, newPoint)
                                        : accessContentFromParagraphPoint(document, newPoint),
                                ),
                                point: newPoint,
                            };
                        }
        ...
        selectionToTranspose.selectionRanges.forEach((selectionRange) => {
            if (selectionRange.ranges.length === 1 && getRangeDirection(document, selectionRange.ranges[0]) === RangeDirection.NeutralText) {
                ...
                selectionRangeUpdates.push([
                    selectionRange,
                    makeRange(
                        collapsedTransposeRange.contentReference,
                        pointAfterFirstGraphemeAfterTransposed,
                        pointAfterFirstGraphemeAfterTransposed,
                        generateId(),
                    ),
                ]);
